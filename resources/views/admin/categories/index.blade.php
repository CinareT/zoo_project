                ...
                <tbody>
                    @foreach ($models as $model)
                        <tr>
                            ...
                            <td class="text-right">
                                <a href="{{ route('admin.categories.show', $model->id) }}" class="btn btn-info mb-1"><i
                                        class="icon fas fa-info mr-2"></i> Info</a>
                                <a href="{{ route('admin.categories.edit', $model->id) }}" class="btn btn-warning mb-1"><i
                                        class="icon fas fa-edit mr-2"></i>
                                    Edit</a>
                                <form onsubmit="return confirm('Are you sure?')" method="post"
                                    action="{{ route('admin.categories.destroy', $model->id) }}" class="d-inline-block">
                                    @method('delete')
                                    @csrf
